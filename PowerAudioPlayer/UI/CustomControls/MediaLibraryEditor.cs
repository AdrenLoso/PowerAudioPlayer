    ...
    public partial class MediaLibraryEditor : UserControl
    {
        ...
        private void cmsKeyWord_Opening(object sender, CancelEventArgs e)
        {
            ...
            for (int i = 0; i < PlaylistHelper.Count; i++)
            {
                ...
                tsitem.Tag = i;
                tsitem.Click += (object? sender, EventArgs e) => { PlaylistHelper.Playlists[Convert.ToInt32(((ToolStripMenuItem)sender).Tag)].Items.AddRange(ItemConvertHelper.ConvertMediaLibraryItemListToPlaylistItemList(result)); };
        ...
        private void tsmiKeyWordSendToNewPlaylist_Click(object sender, EventArgs e)
        {
            if (result.Count != 0)
            {
                ...
                {
                    if (PlaylistHelper.Add(new Playlist(ib.Input) { Items = ItemConvertHelper.ConvertMediaLibraryItemListToPlaylistItemList(result) }) != -1)
                    {
        ...
        private void cmsMediaResult_Opening(object sender, CancelEventArgs e)
        {
            ...
            for (int i = 0; i < PlaylistHelper.Count; i++)
            {
                ...
                ToolStripItem tsitem = new ToolStripMenuItem(PlaylistHelper.Playlists[i].Name);
                tsitem.Tag = i;
                tsitem.Click += (object? sender, EventArgs e) => { PlaylistHelper.Playlists[Convert.ToInt32(((ToolStripMenuItem)sender).Tag)].Items.AddRange(ItemConvertHelper.ConvertAudioInfoListToPlaylistItemList(lvResult.SelectedObjects.Cast<AudioInfo>().ToList())); };
        ...
        private void tsmiMediaSendToNewPlaylist_Click(object sender, EventArgs e)
        {
            if (result.Count != 0)
            {
                ...
                if (ib.ShowDialog() == DialogResult.OK && ib.Input != "")
                {
                    if (PlaylistHelper.Add(new Playlist(ib.Input) { Items = ItemConvertHelper.ConvertAudioInfoListToPlaylistItemList(lvResult.SelectedObjects.Cast<AudioInfo>().ToList()) }) != -1)
                    {
